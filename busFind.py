...
def returnPath(currentNode):
    ...
    for place in path:
        ...
        Coords.append(converted[::-1])
    # attach buses used and reverse array
    path.clear()
    Coords = Coords[::-1]
    # Coords is the bus stop only route
    currIdx = 0
    # newRoute will have the driving route to each bus stop to plot the map easier
    newRoute = []
    while len(Coords) - 1 > currIdx:
        result = pathFind.search(Coords[currIdx], Coords[currIdx + 1])
        newRoute.append(result)
        currIdx += 1
    Coords.clear()
    newRoute.insert(0, busUsed)
    return newRoute
